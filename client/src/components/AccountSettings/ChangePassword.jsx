...
export default function ChangePassword() {
    ...
    const handleSubmit = async (e) => {
        ...
        <div className='space-y-3'>
                ...
                <form onSubmit={handleSubmit} className="p-2 sm:p-3 space-y-2">
                    ...
                    <div className='space-y-1'>
                        ...
                        <div className="relative">
                            <input
                                ...
                                className='w-full bg-gray-50 border border-gray-300 rounded-md p-2 pr-8 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200'
                                required
                            />
                            <button
                                type="button"
                        ...
                        {formData.newPassword && (
                            <div className="flex items-center space-x-1 mt-1">
                                <div className="flex space-x-1 flex-1">
                                    <div className={`h-1 rounded-full flex-1 ${passwordStrength === 'Weak' ? 'bg-red-300' : passwordStrength === 'Medium' ? 'bg-yellow-300' : 'bg-green-300'}`}></div>
                                    <div className={`h-1 rounded-full flex-1 ${passwordStrength === 'Medium' || passwordStrength === 'Strong' ? passwordStrength === 'Medium' ? 'bg-yellow-300' : 'bg-green-300' : 'bg-gray-200'}`}></div>
                                    <div className={`h-1 rounded-full flex-1 ${passwordStrength === 'Strong' ? 'bg-green-300' : 'bg-gray-200'}`}></div>
                                </div>
                    ...
                    <div className='space-y-1'>
                        ...
                        {formData.confirmNewPassword && (
                            <div className={`flex items-center text-xs mt-1 ${formData.newPassword === formData.confirmNewPassword ? 'text-green-600' : 'text-red-600'}`}>
                    ...
                    <div className="pt-2 border-t border-gray-200">
                        <div className="flex justify-end items-center">
                            <button
                                type="submit"
                                className="px-4 py-2 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-md text-sm font-medium hover:from-blue-700 hover:to-indigo-700 transition-all duration-200 shadow-md hover:shadow-lg flex items-center space-x-1"
                            >
            ...
            <div className="bg-gradient-to-r from-amber-50 to-orange-50 rounded-md border border-amber-200 p-2">
                <div className="flex items-start space-x-2">
                    ...
                    <div className="flex-1">
                        <h3 className="text-sm font-semibold text-amber-800 mb-1">Password Requirements</h3>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-1">
                            <div className="flex items-center space-x-1">
                                <div className={`w-1.5 h-1.5 rounded-full ${formData.newPassword.length >= 6 ? 'bg-green-500' : 'bg-gray-300'}`}></div>
                                <span className={`text-xs ${formData.newPassword.length >= 6 ? 'text-green-700' : 'text-gray-600'}`}>
                                    Minimum 6 characters
                                </span>
                            </div>
                            <div className="flex items-center space-x-1">
                                <div className={`w-1.5 h-1.5 rounded-full ${/[A-Z]/.test(formData.newPassword) ? 'bg-green-500' : 'bg-gray-300'}`}></div>
                                <span className={`text-xs ${/[A-Z]/.test(formData.newPassword) ? 'text-green-700' : 'text-gray-600'}`}>
                                    At least one uppercase letter
                                </span>
                            </div>
                            <div className="flex items-center space-x-1">
                                <div className={`w-1.5 h-1.5 rounded-full ${/[!@#$%^&*]/.test(formData.newPassword) ? 'bg-green-500' : 'bg-gray-300'}`}></div>
                                <span className={`text-xs ${/[!@#$%^&*]/.test(formData.newPassword) ? 'text-green-700' : 'text-gray-600'}`}>
